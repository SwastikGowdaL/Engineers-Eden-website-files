...
$GLOBALS['language']->loadDefinitions('all_in_one_shipping', CC_ROOT_DIR.CC_DS.'modules'.CC_DS.'shipping'.CC_DS.$_GET['module'].CC_DS.'language', 'module.definitions.xml');
$lang['all_in_one_shipping'] = $GLOBALS['language']->getStrings('all_in_one_shipping');
...
/* CHECK THAT SETTINGS ARE ACCEPTABLE */
	...
	$module->_settings['use_flat'] = 1;
	$module->module_settings_save($module->_settings);
	$GLOBALS['gui']->setError($lang['all_in_one_shipping']['error_shipping_price_components']);
}
...
if (empty($module->_settings['cc5_data'])) {
	...
	$module->module_settings_save($module->_settings);
	if ($data_upgrade) {
		$GLOBALS['gui']->setNotify($lang['all_in_one_shipping']['notify_data_upgrade']);
	}
}
...
if (isset($_POST['add_rates']) && Admin::getInstance()->permissions('settings', CC_PERM_EDIT)) {
	...
	if ($ins>0 || $upd>0 || $del>0) {
		$GLOBALS['gui']->setNotify(sprintf($lang['all_in_one_shipping']['notify_shipping_rate_changes'], $ins, $upd, $del));
	}
	if ($ins_err>0 || $del_err>0) {
		$GLOBALS['gui']->setError(sprintf($lang['all_in_one_shipping']['error_shipping_rate_changes'], $ins_err, $del_err));
	}
...
if (isset($_GET['delete_zone']) && !empty($_GET['delete_zone']) && Admin::getInstance()->permissions('settings', CC_PERM_DELETE)) {
	if ($GLOBALS['db']->delete('CubeCart_shipping_zones', array('id' => (int)$_GET['delete_zone']))) {
		...
		$GLOBALS['gui']->setNotify($lang['all_in_one_shipping']['notify_shipping_zone_delete']);
	} else {
		$GLOBALS['gui']->setError($lang['all_in_one_shipping']['error_shipping_zone_delete']);
	}
...
if (isset($_POST['zone']) && Admin::getInstance()->permissions('settings', CC_PERM_EDIT)) {
		 ...
		 ($_POST['zone_type']=='S' && !empty($_POST['state_country']) && !empty($_POST['zone_states'])) )
	{
		...
		if (empty($record['zone_name'])) {
			$record['zone_name'] = $lang['all_in_one_shipping']['unnamed_zone'];
		}
		...
		if (empty($_GET['zone_id'])) {
			...
			if ($GLOBALS['db']->insert('CubeCart_shipping_zones', $record)) {
				$GLOBALS['gui']->setNotify($lang['all_in_one_shipping']['notify_shipping_zone_add']);
			} else {
				$GLOBALS['gui']->setError($lang['all_in_one_shipping']['error_shipping_zone_add']);
			}
		} else {
			// Update
			if ($GLOBALS['db']->update('CubeCart_shipping_zones', $record, array('id' => (int)$_GET['zone_id']))) {
				$GLOBALS['gui']->setNotify($lang['all_in_one_shipping']['notify_shipping_zone_update']);
			}
		}

	} else {
		if ($_POST['zone_type']=='C')
			$GLOBALS['gui']->setError($lang['all_in_one_shipping']['error_country_zone']);
		if ($_POST['zone_type']=='S')
			$GLOBALS['gui']->setError($lang['all_in_one_shipping']['error_state_zone']);
		if ($_POST['zone_type']=='P')
			$GLOBALS['gui']->setError($lang['all_in_one_shipping']['error_postcode_zone']);
	}
...
if (isset($_GET['action'])) {
	...
	if($_GET['action'] == 'ajax' && isset($_GET['country_id']) && !empty($_GET['country_id'])) {
		$states = $GLOBALS['db']->select('CubeCart_geo_zone', false, array('country_id' => $_GET['country_id']), array('name' => 'ASC'));
	...
	if ($_GET['action'] == 'edit' && isset($_GET['zone_id']) && is_numeric($_GET['zone_id'])) {
		if (($zone = $GLOBALS['db']->select('CubeCart_shipping_zones', false, array('id' => (int)$_GET['zone_id']), false)) !== false) {
			...
			if (preg_match_all('/\w+/', $zone[0]['states'], $matches)) {
				$template_vars['ZONE_STATES'] = json_encode($matches[0]);
			} else {
				$template_vars['ZONE_STATES'] = json_encode(array());
			}
	...
	//$states = $GLOBALS['db']->select('CubeCart_geo_zone', false, false, array('name' => 'ASC'));
	$template_vars['STATES'] = array(); // This is now ajaxy
	...
	$rest_of_world_zone = array(
		'id' => 0,
		'zone_name' => $lang['all_in_one_shipping']['rest_of_world'],
		'countries' => '',
		'states' => '',
		'postcodes' => '',
		'display_countries' => $lang['all_in_one_shipping']['all_countries'],
		'display_states' => '',
		'display_postcodes' => '',
		'sort_order' => '999',
	...
	if ($zone_records) {
		...
		$GLOBALS['main']->addTabControl($lang['all_in_one_shipping']['tab_shipping_zones'], 'shipping_zones');
		$zones = $zone_records;
	...
	$GLOBALS['main']->addTabControl($lang['all_in_one_shipping']['tab_shipping_zone_add'], null, currentPage(array('tab'), array('action' => 'add')), 'A');
}
